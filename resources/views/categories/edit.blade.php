                ...
                <div class="p-6 text-gray-900">
                    <div class="  flex  items-center justify-center  bg-gray-50">
                        <div>
                          ...
                          <h1 class="mb-1 font-bold text-3xl flex gap-1 items-baseline font-mono">Edit<span class="text-sm text-purple-700">category</sm></h1>
                      <div class="grid max-w-3xl gap-2 py-10 px-8 sm:grid-cols-2 bg-white rounded-md border-t-4 border-purple-400">
                        ...
                        <div class="grid">
                          <div class="bg-white flex min-h-[60px] flex-col-reverse justify-center rounded-md border border-gray-300 px-3 py-2 shadow-sm focus-within:shadow-inner">
                            <input type="text" name="name" value="{{old('name', $category->name)}}" id="name" class="peer block w-full border-0 p-0 text-base text-gray-900 placeholder-gray-400 focus:ring-0" placeholder="name" />
                            ...
                            <div class="bg-white first:flex min-h-[60px] flex-col-reverse justify-center rounded-md border border-gray-300 px-3 py-2 shadow-sm focus-within:shadow-inner">
                              <select name="category_id">
                                  ...
                                  @foreach (App\Models\category::orderBy('name')->pluck('name', 'id')->toArray() as $id => $name)
                                      <option {{$id == old('category_id' , $category->main_categorie->id) ? 'selected' : ''}}
                                      value="{{ $id }}">{{ $name }}</option>
                                  @endforeach
                              </select>
                              <label html="category" class="block transform text-xs font-bold uppercase text-gray-400
                              transition-opacity, duration-200 peer-placeholder-shown:h-0 peer-placeholder-shown:-translate-y-full peer-placeholder-shown:opacity-0">category</label>
                              @error('category')
                              <div class="font-bold text-red-600">{{ $message }}</div>
                          @enderror
